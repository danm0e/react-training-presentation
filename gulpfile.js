...
function renderPresentation() {
    ...
    // Example usage
    const directories = getDirectories(slidesRoot + 'slides/**/*');
    directories.sort();
    const testFiles = [];
    let htmlContent = "";
    directories.forEach((folder) => {
        let myFiles = glob.sync(folder + '/*.{md,html}')
        if(myFiles.length) {
            myFiles.sort();
            htmlContent += "<section>"
    ...
    gulp.src(["./slides/index.html"]) // Every file allown. 
        .pipe(replace('@SLIDES@', htmlContent))
        .pipe(gulp.dest('./'))
