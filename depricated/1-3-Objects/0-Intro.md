...
> INSERT into USERS ...
```
Note: A basic module is just an object with properties applied. These properties can be functions or other types and they can be accessed both within the module and outside of the module.
---
...
> undefined
```
Note: A common pattern in JavaScript modules is the revealing module pattern. This is where you use a function call to return an object that contains only some of the methods and properties used by the module, so you can have pseudo-private methods and properties
---
## Object Constructor
Note: A constructor is function with a set of instructions used when creating an object. This can useful for things like defining instance values or storing any passed configuration.
When a function is called using the new keyword, it is run as a constructor.
---
### The new keyword
- A new object is created
- The constructor is called
- If it returns an object this becomes the result
- Otherwise the onewed object is returned
Note: To run a function as a constructor, we need to use the new keyword. When we do this, the following happens:
...
> Cheese {name: 'Brie'}
```
Note: The below example shows a constructor being called with a value passed to it, which is then stored on the object being created.
...
Note: A prototype is how javascript allows shared functionality. Every object in javascript has a prototype “link” (this can be seen as __proto__ or [[prototype]] in your browser)
These __proto__ links eventually end up back at the parent Object type.
---
### Classes vs Prototypes

Note: Classes are inert, they don’t really do anything and can’t be modified.
...
> Go away Geraldine!
```
Note: Since prototypes are references to an object, if you mutate the object instance that is the parent of your object, you’ll dynamically update the prototype.
---
### Prototypal Inheritance
```js
...
> Not what you're expecting!
```
Note: Built-in object prototype mutation
...
```
Note: In JavaScript, a class is mainly a clearer way of defining a constructor and prototype. The below two examples are equivalent to each other.
---
### Static methods
...
> static constructor
```
Note: In the below example, we’re creating a static method on a constructor and then calling it. You can see that it looks a little cluttered.
---
### Static methods
```js
class ClassExample {
...
> static class
```
Note: Here you can see the equivalent code written as a class. It is far more concise than the constructor example and is easier to understand.
---
### Dynamic Scoping
Note: this in Javascript is dynamic scoping.
...
> { whatIsThis: ƒ }
```
Note: If you call a call a function without having explicitly set this, it will be automatically given a value. This value is assigned to the object that the function is called on.
...
> { hello: true }
```
Note: We can use call, apply or bind to explicitly set the value of this.
...
```
Note: Now we have our first exercise, we’re going to re-implement some lodash methods.
