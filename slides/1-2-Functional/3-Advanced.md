...
---
# Default parameters
By default a function parameter is assigned undefined if no value is passed, however it is often useful to set a different default value. With default parameters we can do this.
...
---
# Spread operator
The spread operator allows iterable objects (such as arrays or strings) and object literals to be expanded where zero or more arguments or elements are expected.
...
---
# Spread operator
One thing to be aware of with the spread operator is that it only creates a new copy of the top level object. Any nested objects will still refer to the same object and will be impacted by modifications.
...
const objA = { nested: {} };
const objB = { ...objA };
objB.nested.hello = true;

console.log(objA);
> { nested: { hello: true } }
console.log(objB);
> { nested: { hello: true } }
var objC = JSON.parse(JSON.stringify(objB));
objB.nested.hello = false;
console.log(objB);
> { nested: { hello: false } }
console.log(objC);
> { nested: { hello: true } }
...
Destructuring
---
We can destructure within objects, to save us having to traverse deeper into the object.
